...
class BedAllocation(models.Model):
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.PROTECT, related_name="bed_allocations"
    )
    admission_date = models.DateTimeField(auto_now_add=True)
    discharge_date = models.DateTimeField(null=True, blank=True)
    reason = models.TextField(blank=True)
    status = models.CharField(
        max_length=20, choices=ALLOCATION_STATUS, default="ACTIVE"
    )
...
class BedRequest(models.Model):
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="bed_requests"
    )
    doctor = models.ForeignKey(
        Doctor, on_delete=models.CASCADE, related_name="bed_requests"
    )
    appointment = models.ForeignKey(
        Appointment, on_delete=models.CASCADE, related_name="bed_requests"
    )
