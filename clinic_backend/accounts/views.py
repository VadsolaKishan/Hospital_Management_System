...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def google_login(self, request):
        ...
        try:
            # Verify the access token with Google to get user info
            user_info_response = api_requests.get(
            ...
            if not email:
                return Response(
                    {"error": "Google token invalid: no email"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def create_doctor(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                return Response(
                    {"message": "Doctor account created successfully", "email": email},
                    status=status.HTTP_201_CREATED,
                )
...
Hospital Management System
    ...
    def verify_reset_token(self, request):
        ...
        if serializer.is_valid():
            return Response(
                {"message": "Token is valid", "valid": True}, status=status.HTTP_200_OK
            )
