...
export const BedList = () => {
    ...
    // Queries
    const { data: wards } = useQuery({ queryKey: ['wards'], queryFn: bedService.getWards, gcTime: 0, staleTime: 0 });
    const { data: patients } = useQuery<Patient[]>({ queryKey: ['patients'], queryFn: () => patientService.getAll() });

    const { data: allBeds, isLoading } = useQuery({
        queryKey: ['beds'],
    ...
    const createMutation = useMutation({
        ...
        onError: (err: ApiError | unknown) => {
            ...
            toast({ title: 'Error', description: error.response?.data?.detail || 'Failed to add bed', variant: 'destructive' });
        }
    });
    ...
    const admitMutation = useMutation({
        ...
        onError: (err: ApiError | unknown) => {
            const error = err as ApiError;
            toast({ title: 'Error', description: error.response?.data?.detail || 'Failed to admit patient', variant: 'destructive' });
        }
    });
    ...
    const dischargeMutation = useMutation({
        ...
        onError: (err: ApiError | unknown) => {
            const error = err as ApiError;
            toast({ title: 'Error', description: error.response?.data?.error || 'Failed to discharge patient', variant: 'destructive' });
        }
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        createMutation.mutate({
            ...
            is_active: true,
            bed_type: formData.bed_type as Bed['bed_type']
        } as Partial<Bed>);
    };
                            ...
                            <tr
                                ...
                                onClick={() => {
                                    if (bed.status === 'OCCUPIED' && bed.current_allocation) {
                                        ...
                                        const patientId = typeof bed.current_allocation === 'object' && bed.current_allocation !== null ? (bed.current_allocation as { patient: number }).patient : bed.current_allocation;
                                    ...
                                    {bed.status === 'OCCUPIED' && bed.current_allocation ? (
                                        <div className="flex flex-col">
                                            <span className="font-medium text-slate-900">
                                                {bed.current_allocation.patient_name || bed.current_allocation.patient_details?.user_name || 'Patient'}
                                            </span>
                                            {bed.current_allocation.patient_uhid && (
